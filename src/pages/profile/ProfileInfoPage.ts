...
class ProfileInfoPage extends BaseBlock {
    ...
    public static create (profile: Profile) {
        return new ProfileInfoPage({
            ...
            links: [
                new Link({href: '#profile-redact', text: 'Изменить данные', class: 'profile-link profile-redact-link'}),
                new Link({href: '#profile-change-password', text: 'Изменить пароль', class: 'profile-link'}),
                new Link({href: '#authorization', text: 'Выйти', class: 'profile-link profile-exit-link'}),
            ],
        })
    }
}
